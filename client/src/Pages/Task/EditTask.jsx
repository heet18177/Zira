...
import { updateTask, getTaskById } from "@/Redux/thunk/taskManage";
import { projects as fetchProjects } from "@/Redux/thunk/projectManage";
...
const EditTask = () => {
  ...
  const currentTask = Array.isArray(tasks) ? tasks?.find(t => t._id === id) : (tasks?._id === id ? tasks : null);
  ...
  const filterAssign = [];
  if (Array.isArray(projectUser) && currentTask) {
    const project = projectUser.find(p => p._id === currentTask.projectId);
  ...
  useEffect(() => {
    if (currentTask) {
      setForm({
        ...
        status: currentTask.taskStatus || "Pending",
        priority: currentTask.taskPriority || "Medium",
        startDate: currentTask.startDateT ? new Date(currentTask.startDateT).toISOString().split("T")[0] : "",
        deadline: currentTask.dueDateT ? new Date(currentTask.dueDateT).toISOString().split("T")[0] : "",
        assignTo: currentTask.assignTo?.map(a => typeof a === 'object' ? a._id : a) || [],
      });
    }
  }, [currentTask]);
...
if (result.meta.requestStatus === "fulfilled") {
  ...
  navigate(`/view-tasks/${currentTask?.projectId}`);
} else {
  toast.error(
    ...
    <>
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <button
                onClick={() => navigate(`/view-tasks/${currentTask?.projectId}`)}
          ...
          <p className="mt-2 text-base text-slate-500 sm:ml-[52px]">
            Modify the details below to update your task.
          </p>
        </div>
